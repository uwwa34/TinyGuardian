...
class TallyScreen {
  ...
  draw(ctx){
    ...
    for(let i=0;i<Math.min(this.animIndex+1,this.lines.length);i++){
      ...
      ctx.font='18px serif';ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillStyle=COL.PRIMARY_D;ctx.fillText(l.icon,42,ry+4);
      ctx.font='15px '+FONT.BODY;ctx.fillStyle=COL.HUD_TEXT;ctx.fillText(l.label,68,ry+4);
      ctx.font='bold 15px '+FONT.BODY;ctx.textAlign='right';ctx.fillText(l.val,WIDTH-46,ry+4);
      ry+=rowH;
    }
    ctx.globalAlpha=1;ry+=4;
    ctx.strokeStyle=COL.PRIMARY_D;ctx.lineWidth=1;ctx.setLineDash([4,4]);ctx.beginPath();ctx.moveTo(32,ry);ctx.lineTo(WIDTH-32,ry);ctx.stroke();ctx.setLineDash([]);ry+=14;
    ...
    ctx.fillText('รวมทั้งหมด',44,ry+24);ctx.textAlign='right';ctx.fillText('⭐ '+this.total.toLocaleString(),WIDTH-44,ry+24);ry+=60;
    if(this.done){
...
class NameScreen {
  ...
  handleKey(key){
    ...
    else if(key==='Enter') return (this.name.trim()||'GUARDIAN');
  ...
  draw(ctx){
    ...
    ctx.fillStyle='rgba(255,248,225,0.92)';ctx.strokeStyle=COL.PRIMARY;ctx.lineWidth=2;_rr(ctx,16,52,WIDTH-32,250,18);ctx.fill();ctx.stroke();
    ctx.fillStyle=COL.HUD_TEXT;ctx.font='bold 22px '+FONT.MAIN;ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText('✏️ ใส่ชื่อผู้เล่น',WIDTH/2,85);
    ctx.fillStyle=COL.PRIMARY_D;ctx.font='14px '+FONT.BODY;ctx.fillText('ไม่ใส่ชื่อ = GUARDIAN',WIDTH/2,112);
    const bx=WIDTH/2-150,by=140;
    ctx.fillStyle='#FFF';ctx.strokeStyle=COL.PRIMARY;ctx.lineWidth=2;_rr(ctx,bx,by,300,50,10);ctx.fill();ctx.stroke();
    ctx.fillStyle=this.name?COL.HUD_TEXT:'#9E9E9E';ctx.font='bold 22px "Courier New",monospace';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText((this.name||'GUARDIAN')+(this.cursor?'|':' '),WIDTH/2,by+25);
    this._drawVKB(ctx,310);
    ctx.fillStyle=COL.PRIMARY;_rr(ctx,WIDTH/2-105,460,210,46,14);ctx.fill();ctx.strokeStyle=COL.PRIMARY_D;ctx.lineWidth=2;_rr(ctx,WIDTH/2-105,460,210,46,14);ctx.stroke();
    ctx.fillStyle=COL.HUD_TEXT;ctx.font='bold 17px '+FONT.MAIN;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('✅ ยืนยัน',WIDTH/2,483);
  }
  _drawVKB(ctx,startY){
    const rows=[['Q','W','E','R','T','Y','U','I','O','P'],['A','S','D','F','G','H','J','K','L'],['Z','X','C','V','B','N','M','⌫']];
    const keyH=42,keyGap=3;this._kbData={rows,startY,keyH,keyGap};
...
class RankingScreen {
  ...
  addScore(name,score){
    ...
    const entry={name:(name||'GUARDIAN').trim().slice(0,10).toUpperCase(),score,date:new Date().toLocaleDateString('th-TH')};
    this.entries.push(entry);this.entries.sort((a,b)=>b.score-a.score);this.entries=this.entries.slice(0,RANKING_MAX);
  ...
  draw(ctx){
    ...
    ctx.fillStyle=g;ctx.fillRect(0,0,WIDTH,HEIGHT);
    ['🌟','✨','⭐','🌟'].forEach((e,i)=>{const px=[22,WIDTH-28,48,WIDTH-52][i],py=[44,44,HEIGHT-50,HEIGHT-50][i];ctx.font='18px serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.globalAlpha=0.4+Math.sin(Date.now()/700+i)*0.3;ctx.fillText(e,px,py);});ctx.globalAlpha=1;
    ctx.fillStyle='rgba(255,248,225,0.94)';ctx.strokeStyle=COL.PRIMARY;ctx.lineWidth=2;_rr(ctx,12,56,WIDTH-24,HEIGHT-140,18);ctx.fill();ctx.stroke();
    ctx.fillStyle=COL.HUD_TEXT;ctx.font='bold 20px '+FONT.MAIN;ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText('🏆 Tiny Guardian TOP 10 🏆',WIDTH/2,86);
    const list=this.entries;const rowH=46,startY=112;const medals=['🥇','🥈','🥉'];
    for(let i=0;i<RANKING_MAX;i++){
      const y=startY+i*rowH,isMe=i===this.highlightIndex,entry=list[i];
      // Row background
      if(isMe){ctx.fillStyle='rgba(255,236,179,0.5)';_rr(ctx,18,y+2,WIDTH-36,rowH-4,8);ctx.fill();ctx.strokeStyle=COL.PRIMARY_D;ctx.lineWidth=1.5;_rr(ctx,18,y+2,WIDTH-36,rowH-4,8);ctx.stroke();}
      // Stagger reveal — always visible after 1 second
      const reveal = Math.min(1, (this._t - i*3) / 10);
      ctx.globalAlpha = Math.max(0.05, reveal);  // minimum 5% so text always somewhat visible
      // Medal
      ctx.fillStyle=COL.HUD_TEXT; ctx.font='17px serif';ctx.textAlign='left';ctx.textBaseline='middle';
      ctx.fillText(medals[i]||((i+1)+'.'),22,y+rowH/2);
      if(entry){
        ctx.fillStyle=isMe?COL.HUD_TEXT:'rgba(93,64,55,0.8)';ctx.font=(isMe?'bold ':'')+'14px "Courier New",monospace';ctx.textAlign='left';
        ctx.fillText(entry.name+(isMe?' ◀':''),54,y+rowH/2-5);
        ctx.fillStyle='#BFA030';ctx.font='10px '+FONT.BODY;ctx.fillText(entry.date||'',54,y+rowH/2+9);
        ctx.fillStyle=COL.PRIMARY_D;ctx.font='bold 16px "Courier New",monospace';ctx.textAlign='right';ctx.fillText(entry.score.toLocaleString(),WIDTH-22,y+rowH/2);
      } else {
        ctx.fillStyle='rgba(93,64,55,0.3)';ctx.font='14px '+FONT.BODY;ctx.textAlign='left';ctx.fillText('---',54,y+rowH/2);
      }
      ctx.globalAlpha=1;
    }
    const by=HEIGHT-80;
    ctx.fillStyle=COL.PRIMARY;_rr(ctx,WIDTH/2-120,by,240,50,14);ctx.fill();ctx.strokeStyle=COL.PRIMARY_D;ctx.lineWidth=2;_rr(ctx,WIDTH/2-120,by,240,50,14);ctx.stroke();
    ctx.fillStyle=COL.HUD_TEXT;ctx.font='bold 17px '+FONT.MAIN;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('🌟 เล่นอีกครั้ง!',WIDTH/2,by+25);
  }
}
